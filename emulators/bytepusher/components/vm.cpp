...
uint32_t BYTEPUSHER_VM::getVal(uint32_t pc, uint32_t n)
{
    uint32_t v = 0;
    while(n--)
        v = (v << 8) + ram[pc++];
...
int8_t BYTEPUSHER_VM::writeBuffer(uint16_t offset, uint8_t* buffer, uint32_t size)
{
    // Safety checks
...
uint32_t BYTEPUSHER_VM::getState()
{
    return state;
}

void BYTEPUSHER_VM::draw(SDL_Texture* output, SDL_Renderer* tool)
{
...
void BYTEPUSHER_VM::update(double dt, Window* win)
{
    // Update delta time
    counter += dt;
