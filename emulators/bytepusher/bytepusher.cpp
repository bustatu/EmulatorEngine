...
void BYTEPUSHER_EMU::update(double dt)
{
...
void BYTEPUSHER_EMU::resume()
{
    printf("{I}: Bytepusher emulator has resumed!\n");
    is_running = true;    
}

void BYTEPUSHER_EMU::pause()
{
    printf("{I}: Bytepusher emulator has been stopped!\n");
    is_running = false;    
}

void BYTEPUSHER_EMU::load(std::string path)
{
    // Create new VM
    if(vm != nullptr)
        delete vm;
    vm = new BYTEPUSHER_VM();
